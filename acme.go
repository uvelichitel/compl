...
var Marcs []byte = []byte{'!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '=', '+', '[', ']', '{', '}', '|', ';', ':', '\'', '"', '\b', '\n', '\r', '\t', '\\', '<', '>','?', ',', '.', ' ', }
var marks []uint8 = []uint8(Marcs)
...
func acmeCurrentFile() (*acmeFile, error) {
	...
	for i:=q0-1; slice.ContainsUInt8(marks, uint8(body[i])) == false; i-- {
	q0exp=i
	}
	
	for i:=q1; slice.ContainsUInt8(marks, uint8(body[i])) == false; i++ {
	q1exp=i+1
	}
	
	if err != nil {
		return nil, fmt.Errorf("cannot set dot=addr: %v", err)
	}
